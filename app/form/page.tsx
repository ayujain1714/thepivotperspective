...
export default function Home() {
  ...
  const getSeatClass = (
    ...
    let seatClass = 'h-[30px] w-[30px] cursor-pointer rounded-[7px] border border-white bg-transparent text-center text-[7px] leading-[30px] transition-all'; // Replace with your default class
  
    if (status === 'Booked') {
      seatClass += ' animate-vibrate border-[#ff5959cc] bg-[#ff5959cc] font-bold';
    } else if (status === 'Pending') {
      seatClass += ' animate-vibrate border-[#faff69a2] bg-[#faff69a2] font-bold';
    }
  
    if (selectedSeat?.partIndex === partIndex &&
        selectedSeat?.rowIndex === rowIndex &&
        selectedSeat?.seatIndex === seatIndex) {
      seatClass += ' animate-vibrate border-[#4fffb391] bg-[#4fffb391] font-bold';
    }

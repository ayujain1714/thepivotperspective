...
export default function Home() {
  ...
  const getSeatClass = (
    ...
    if (status === 'Booked') {
      seatClass +=
        ' animate-vibrate pointer-events-none border-slate-700 bg-slate-700 font-bold';
    } else if (status === 'Pending') {
      seatClass +=
        ' animate-vibrate pointer-events-none border-[#faff69a2] bg-yellow-500 font-bold';
    }
  ...
  return (
    <>
      <div className="main border-box flex h-fit w-full flex-col">
        <div className="mx-auto mb-[70px] box-border h-[100px] w-[80vw] place-content-center rounded-b-xl border-2 border-[#49494975] px-2 text-center shadow-[0_35px_60px_-15px_rgba(255,255,255,0.2)] md:h-[250px] md:w-[80vw] md:border-4">
        ...
        <div className="m-auto mb-2 flex h-fit w-full overflow-x-auto overflow-y-hidden">
          <div className="seat-layout border-box mx-auto flex h-fit w-fit flex-row gap-[30px] p-[18px]">
            {seatLayout.map((part, partIndex) => (
              ...
              >
                <div className="seat-row mx-auto flex flex-wrap justify-center gap-[5px] px-4">
                  {Array.from({ length: 9 }).map((_, index) => (
                    <div
                      className="pointer-events-none h-[20px] w-[40px] cursor-pointer rounded-[7px] border border-slate-700 bg-slate-700 text-center text-[5px] leading-[20px] transition-all md:h-[30px] md:w-[60px] md:text-[7px] md:leading-[30px]"
                      key={index}
                    >
                      Couch
                    </div>
                ...
                <hr />
                <div className="seat-row mx-auto mb-5 flex flex-wrap justify-center gap-[5px] px-4">
                  {Array.from({ length: 30 }).map((_, index) => (
                    <div
                      className="h-[20px] w-[20px] rounded-[7px] border border-slate-700 bg-slate-700 text-center text-[5px] leading-[20px] transition-all md:h-[30px] md:w-[30px] md:text-[7px] md:leading-[30px]"
                      key={index}
                    >
                      Free
                    </div>
        ...
        <div className="items-center justify-around text-center max-sm:space-y-4 sm:flex">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-sm">
              ...
              <a href='https://www.linkedin.com/in/ayush-jain-54b285290/' target="_blank" className="font-bold">Ayush Jain</a> and <a href='https://www.linkedin.com/in/mainishanhoon/' target="_blank" className="font-bold">Nishan Kashyap</a>.<br/>All rights reserved.
            </p>
          </div>
